...
class RenderParamsTest {
    ...
    fun `deserialize JSON to RenderParams with custom values`() {
        ...
        assertEquals(listOf("depth", "normal"), params.image_synthesis)
        assertEquals(true, params.find_solution)
        assertEquals(true, params.save_pose_data)
        assertEquals(true, params.save_scene_status)
        assertEquals(listOf("MANUAL"), params.camera_mode)
        assertEquals(true, params.recording)
        assertEquals(1920, params.image_width)
        assertEquals(1080, params.image_height)
        assertEquals(0.5, params.time_scale)
        assertEquals(true, params.skip_animation)
        assertEquals(true, params.vis_check_object)
        assertEquals(true, params.vis_check_character)
        assertEquals(true, params.vis_check_object_all)
        assertEquals(true, params.out_graph)
        assertEquals(10, params.per_frame)
        assertEquals(listOf("Cam1", "Cam2"), params.specified_cameras)
        assertEquals(listOf("Diag1"), params.diagonal_cameras1)
        assertEquals(listOf("Diag2"), params.diagonal_cameras2)
    }
}
