...
//        explicitNulls = false
    ...
    fun `serialize ExpandSceneConfig to JSON with default values`() {
        ...
        assertEquals(
            """{"randomize":false,"random_seed":-1,"animate_character":false,"ignore_obstacles":false,"transfer_transform":true}""",
            json,
        )
    }
    ...
    fun `deserialize JSON to ExpandSceneConfig with custom values`() {
        val json = """{"randomize":true,"random_seed":42,"animate_character":true,"ignore_obstacles":true,"transfer_transform":false}"""
